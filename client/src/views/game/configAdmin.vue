<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      >
      <el-select
        v-model="listQueryTask.condition.account"
        :placeholder="'代理'"
        size="small"
        clearable
        class="filter-item"
      ...
      >
        <el-option
          v-for="item in calendarTypeOptionsUser"
          :key="item.name"
          :label="item.name"
          :value="item.name"
        />
    ...
    >
      ...
      <el-table-column label="代理">
        <template slot-scope="scope">{{
          scope.row.user ? scope.row.user : scope.row.account
        }}</template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="代理">
          <el-select v-model="user" class="filter-item" placeholder="请选择">
            <el-option
              v-for="item2 in calendarTypeOptionsUser"
              :key="item2.name"
              :label="item2.name"
              :value="item2.name"
            />
...
const calendarTypeOptionsUser: any = [];
...
export default class extends Vue {
  ...
  private calendarTypeOptionsUser = calendarTypeOptionsUser;
  ...
  private async getUSerList() {
    ...
    this.calendarTypeOptionsUser = data.items;
  }
  ...
  private async getList() {
    ...
    // todo 暂时使用auth数据显示
    const { data } = await getTimeTaskAll(this.listQueryTask);
  ...
  private async clickSelectPhone() {
    ...
    // 加载分组数据
    const gdata = await getGrouping({ user: this.user });
    this.listGrouping = gdata.data.items;
    // 加载设备数据
    const { data } = await getGamesByAccount({ user: this.user });
  ...
  private async selectChange() {
    ...
    this.serveList = [];
    this.clientList = [];
    let sData = this.selectGroupingData.filter(
      (item: any) => String(item.grouping).indexOf(this.selectG) !== -1
    );
    sData.forEach((v: any) => {
      if (v.type == "image") {
        // 图片素材
